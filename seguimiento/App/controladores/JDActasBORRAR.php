...
public function descargar_actas_csv() {
    ...
    foreach ($dataGrouped as $evaluacion => $ciclos) {
        foreach ($ciclos as $ciclo => $cursos) {
            ...
            // Título de bloque
            $titulo = utf8_decode("$evaluacion - $ciclo ($grado - $turno)");
...
public function descargar_actas_ep1() {
    ...
    foreach ($ciclos as $nombre_ciclo => $modulos) {
        ...
        // Obtener grado (nombre) y turno desde el primer objeto disponible
                ...
                foreach ($this->datos['preguntas_ep1'] as $pregunta) {
                    ...
                    foreach ($respuestas_modulo as $respuesta) {
                        if ($respuesta->id_pregunta == $pregunta->id_pregunta && isset($respuesta->ep1)) {
                            $respuestas_pregunta[] = floatval($respuesta->ep1);
                        }
                    }

                    if (!isset($respuestas_totales[$pregunta->id_pregunta])) {
                        $respuestas_totales[$pregunta->id_pregunta] = [];
                    }

                    $respuestas_totales[$pregunta->id_pregunta] = array_merge(
                        $respuestas_totales[$pregunta->id_pregunta],
                        $respuestas_pregunta
                    );
...
public function descargar_actas_csv_ep1() {
    ...
    foreach ($ciclos as $nombre_ciclo => $modulos) {
        ...
        // Título de bloque (convertido)
        fputcsv($output, []);
        fputcsv($output, [utf8_decode($nombre_ciclo . " (" . $grado . " - Turno " . $turno . ")")]);
        fputcsv($output, []);
        ...
        foreach ($cursos as $numero_curso => $modulos_curso) {
            ...
            // Codificar promedio
            $fila_promedio = array_map(function ($val) {
                return is_string($val) ? utf8_decode($val) : $val;
            }, $fila_promedio);
