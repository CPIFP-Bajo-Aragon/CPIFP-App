    ...
    <div class="row d-flex justify-content-center text-center mx-0 mt-3">
        <div class="col-12">
            ...
            <?php if(isset($datos['persistencia']['nombreDestinoSeleccionado'])):
                    echo "Destino seleccionado: ".$datos['persistencia']['nombreDestinoSeleccionado'];
            endif ?>

            <?php if (isset($datos['destinos']) && count($datos['destinos'])>1): ?>
                    <form action="GestionFacturas" method="POST">
                    Cambiar destino facturación:  <select name="destino">
                            <?php foreach($datos["destinos"] as $destino): ?>
                                <option value=<?php echo $destino->Destino_Id?>><?php echo $destino->Depart_Servicio?></option>
                            <?php endforeach ?>
                    </select>
                             <input type="submit" name="seleccionar" value="Seleccionar destino">   
                    </form>
